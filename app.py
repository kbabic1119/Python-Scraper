...
# ─── SOURCE CONFIG ───────────────────────────────────────────────────────────
SOURCE_CONFIG = {
    "duckduckgo":  {"label": "🦆 DuckDuckGo",   "min": 5, "max": 100, "default": 30,  "cls": "active-ddg",    "desc": "Free web search — no API key needed."},
    "serpapi":     {"label": "🔎 SerpApi",       "min": 5, "max": 100, "default": 50,  "cls": "active-serp",   "desc": "Paid Google search API — most reliable. Requires SerpApi key."},
    "gemini":      {"label": "🤖 Gemini AI",    "min": 5, "max": 200, "default": 50,  "cls": "active-gemini", "desc": "Uses Gemini AI to discover leads — 200 leads/call."},
    "google_maps": {"label": "🗺 Google Maps",  "min": 5, "max": 20,  "default": 20,  "cls": "active-maps",   "desc": "Real verified local businesses with addresses. Limit: 20."},
}

if "search_source" not in st.session_state:
    st.session_state["search_source"] = "duckduckgo"
...
# ─── 3-TAB LAYOUT ─────────────────────────────────────────────────────────────
# ═══════════════════════════════════════════════════════════════════════════════
tab_discover, tab_process, tab_results = st.tabs(["🔍 Discovery", "⚡ Processing", "📊 Results & Management"])

# ═══════════════════════════════════════════════════════════════════════════════
# TAB 1: DISCOVERY
# ═══════════════════════════════════════════════════════════════════════════════
with tab_discover:
    st.markdown("<p style='font-family:JetBrains Mono,monospace;color:#00d4ff;font-size:0.8rem;letter-spacing:2px;'>// FIND NEW LEADS</p>", unsafe_allow_html=True)
    ...
    with st.container(border=True):
        ...
        if search_query:
            st.markdown(f"<span class='src-desc'>🔎 Query: <strong>{search_query}</strong></span>", unsafe_allow_html=True)
    ...
    with st.container(border=True):
        ...
        for i, (src_key, src_cfg) in enumerate(SOURCE_CONFIG.items()):
            with btn_cols[i]:
                ...
                if st.button(src_cfg["label"], key=f"src_{src_key}", use_container_width=True, type="primary" if is_active else "secondary"):
        ...
        lead_limit = st.slider(
            f"Leads to find (max {cfg['max']})",
            min_value=cfg["min"], max_value=cfg["max"],
            value=min(cfg["default"], cfg["max"]),
        )

    # Big Find Leads button
    run_finder = st.button("🔍 FIND LEADS", use_container_width=True, type="primary")
    if run_finder:
        if not search_query:
...
<div style='text-align:center;padding:40px 20px;border:1px dashed #0e2040;border-radius:16px;margin-top:20px;'>
  ...
  <p style='font-family:JetBrains Mono,monospace;color:#00d4ff;font-size:0.9rem;letter-spacing:3px;'>NO LEADS YET</p>
  <p style='color:#2a4060;font-size:0.82rem;'>Pick a template or enter a query above, then click <strong style='color:#4a80c0'>Find Leads</strong>.</p>
  <div style='margin-top:16px; text-align:left; max-width:480px; margin-left:auto; margin-right:auto;'>
    <p style='font-family:JetBrains Mono,monospace;color:#4a6080;font-size:0.7rem;letter-spacing:1px;margin-bottom:8px;'>EXAMPLE QUERIES:</p>
    <p style='color:#3a5a80;font-size:0.8rem;margin:3px 0;'>• HVAC companies Oslo Norway</p>
    <p style='color:#3a5a80;font-size:0.8rem;margin:3px 0;'>• Dental clinics Stockholm Sweden</p>
    <p style='color:#3a5a80;font-size:0.8rem;margin:3px 0;'>• Plumbing services London UK</p>
    <p style='color:#3a5a80;font-size:0.8rem;margin:3px 0;'>• Real estate agencies Miami Florida</p>
  </div>
...
with tab_process:
    ...
    # ── PANEL 1: PRE-SCORE (Free) ──────────────────────────────────────
    with proc_col1:
        with st.container(border=True):
            st.markdown("<p style='font-family:JetBrains Mono,monospace;color:#ffb800;font-size:0.8rem;letter-spacing:2px;'>// PRE-SCORE ENGINE</p>", unsafe_allow_html=True)
            st.markdown("<span class='src-desc'>🆓 FREE — No API keys needed. 16 pain-point checks using Python.</span>", unsafe_allow_html=True)

            scored_n = get_count("pain_scored_leads.csv")
            leads_n = get_count("leads.csv")
            if scored_n > 0:
                st.markdown(f"<span class='step-badge step-scored'>✓ {scored_n} leads scored</span>", unsafe_allow_html=True)
            elif leads_n > 0:
                st.info(f"**{leads_n}** leads ready to score")
            else:
                st.caption("No leads found yet — go to Discovery tab first.")

            st.markdown("")
            run_prescorer = st.button("⚡ PRE-SCORE ALL LEADS", use_container_width=True, type="primary", disabled=(leads_n == 0))

            if run_prescorer:
                st.session_state["last_error"] = None
                with st.status("⚡ Pre-Scoring Leads...", expanded=True):
                    progress_bar = st.progress(0)
                    run_and_stream([sys.executable,"pre_scorer.py"], "PRE-SCORING LEADS (free, 16 checks)", progress_bar, 1, 1)
                    progress_bar.progress(1.0)
                st.rerun()

            st.markdown("")
            st.markdown("<p style='font-family:JetBrains Mono,monospace;color:#2a3a5a;font-size:0.65rem;letter-spacing:1px;'>CHECKS: SSL, CMS, Mobile, Chatbot, Booking, Analytics, Schema, Open Graph, Social Links, Contact Forms, Response Time, Domain Age, Email Provider, Wayback Staleness, Robots.txt, HTTP Headers</p>", unsafe_allow_html=True)

    # ── PANEL 2: DEEP ANALYSIS (API Keys) ──────────────────────────────
    with proc_col2:
        with st.container(border=True):
            st.markdown("<p style='font-family:JetBrains Mono,monospace;color:#bf7fff;font-size:0.8rem;letter-spacing:2px;'>// DEEP ANALYSIS</p>", unsafe_allow_html=True)
            st.markdown("<span class='src-desc'>🔑 Uses Playwright + Gemini AI — requires API keys for AI step.</span>", unsafe_allow_html=True)

            enriched_n = get_count("enriched_leads.csv")
            analyzed_n = get_count("deep_extracted_leads.csv")

            # Status indicators
            if enriched_n > 0:
                st.markdown(f"<span class='step-badge step-scraped'>✓ {enriched_n} sites scraped</span>", unsafe_allow_html=True)
            if analyzed_n > 0:
                st.markdown(f"<span class='step-badge step-analyzed'>✓ {analyzed_n} AI analyzed</span>", unsafe_allow_html=True)

            st.markdown("")
            scrape_col, analyze_col = st.columns(2)
            with scrape_col:
                can_scrape = leads_n > 0
                run_diver = st.button("🌐 Scrape Sites", use_container_width=True, disabled=not can_scrape)
            with analyze_col:
                can_analyze = enriched_n > 0
                run_analyzer = st.button("🧠 AI Analyze", use_container_width=True, disabled=not can_analyze)
            ...
            st.markdown("")
            st.markdown("<p style='font-family:JetBrains Mono,monospace;color:#2a3a5a;font-size:0.65rem;letter-spacing:1px;'>SCRAPE: Visits each site with Playwright/requests. ANALYZE: Gemini AI extracts business intelligence.</p>", unsafe_allow_html=True)

    # ── PAIN SCORE VISUALIZATION ────────────────────────────────────────
    _scored_df = safe_read_csv("pain_scored_leads.csv")
    if len(_scored_df) > 0 and "Pain Score" in _scored_df.columns:
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("<p style='font-family:JetBrains Mono,monospace;color:#ffb800;font-size:0.8rem;letter-spacing:2px;'>// PAIN SCORE DISTRIBUTION</p>", unsafe_allow_html=True)
        v1, v2 = st.columns([3, 2])
        with v1:
            pain_counts = pd.to_numeric(_scored_df['Pain Score'], errors='coerce').value_counts().sort_index()
            full_pain_counts = pd.Series(0, index=range(1, 11)).add(pain_counts, fill_value=0)
            st.bar_chart(full_pain_counts, color="#ffb800")
        with v2:
            try:
                avg_pain = pd.to_numeric(_scored_df['Pain Score'], errors='coerce').mean()
                high_pain = len(_scored_df[pd.to_numeric(_scored_df['Pain Score'], errors='coerce') >= 7])
            except Exception:
                avg_pain = 0
                high_pain = 0
            st.markdown(f"""
...
with tab_results:
    ...
    if df is not None and len(df) > 0:
        ...
        # --- MARKET PULSE VISUALIZATION (deep extracted) ---
        ...
        hot_leads_count = 0
        if data_label == "deep_extracted":
            try: hot_leads_count = len(df[pd.to_numeric(df['Automation Score'], errors='coerce') >= 8])
            except Exception: pass
        elif data_label == "pain_scored" and "Pain Score" in df.columns:
            try: hot_leads_count = len(df[pd.to_numeric(df['Pain Score'], errors='coerce') >= 7])
            except Exception: pass
        ...
        with ctrl2:
            if data_label == "deep_extracted":
                ...
                min_score = st.slider("Min Pain Score", 1, 10, 6, help="Only show leads with pain score >= this value")
            else:
                min_score = 1
        with ctrl3:
        ...
        if data_label == "deep_extracted" and min_score > 1:
            ...
            if '_score_num' in df.columns: df = df.drop(columns=['_score_num'])
        ...
        for chunk in chunks:
            ...
            for col, (row_idx, lead) in zip(cols, chunk.iterrows()):
                with col:
                    # Selection checkbox
                    is_checked = select_all or (row_idx in st.session_state["selected_leads"])
                    ...
                    if is_valid(email):
                        for e in str(email).split(",")[:2]:
                            e = e.strip()
                            if e: email_pills += f"<a href='mailto:{e}' target='_blank' class='info-pill has-data' style='text-decoration:none;'>✉ {e[:30]}</a> "

                    phone_pills = ""
                    if is_valid(phone):
                        for p in str(phone).split(",")[:2]:
                            p = p.strip()
                            if p: phone_pills += f"<a href='tel:{p.replace(' ','')}' class='info-pill has-data' style='text-decoration:none;'>📞 {p[:20]}</a> "
